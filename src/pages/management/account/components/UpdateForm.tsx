...
class UpdateForm extends Component<UpdateFormProps, UpdateFormState> {
  ...
  renderFooter = (currentStep: number) => {
    const { handleUpdateVisible, values } = this.props;
    if (currentStep === 1) {
      return [
        ...
        <Button
          key='cancel'
          onClick={() => handleUpdateVisible(false, values)}
        >
          取消
        </Button>,
        <Button
  ...
  render() {
    const { updateVisible, handleUpdateVisible, values } = this.props;
    ...
    return (
      <Modal
        ...
        onCancel={() => handleUpdateVisible(false, values)}
        afterClose={() => handleUpdateVisible()}
      >
